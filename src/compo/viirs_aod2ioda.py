...
def get_confidence_qc_mask(qfarr,aerosol_type,conf_lvl='MedHigh'):
    ...
    #Applying more than qf here doesn't affect the final results so long as all other masks are applied, 
    #but was useful to validate the number of smoke obs of each confidence level. 
    
    #smoke confidence is defined in (counting from 0) bit 2-3, dust bit 4-5 
    ...
    return conf_mask #actually, true elements here are values we want to keep (invert for an actual mask)
    ...
    def _read(self):
        ...
        for f in self.filenames:
            ...
            if self.adp_mask:
                ...
                #Note: this works for the directory structure that exists for these
                #files on hera, but if the path or filename contains multiple 
                #instances of one of the replaced strings it could cause 
                #problems. A safer approach would be to have the
                #masked output filenames specified as a CLI argument
                
                #Apparently the creation time can differ between the AOD and ADP filenames,
                #so the below line doesn't always work to get the ADP filename
                #adp_fn = f.replace('aod', 'adp').replace("JRR-AOD", "JRR-ADP")
                
                #two options: 1.) instead of operating on full filename, break it up and reconstruct it from pieces
                #             2.) glob filenames that match everything before the c marking the creation time;
                #                 there should only be one for each AOD file 
                ...
                if len(adp_fn)==1:
                  ...
                  exit(f'ERROR: Too many ADP files found for AOD file')
                ...
                #apply masks from ADP data (no qc, leave here for now in case qc becomes an option)
                # vals_smoke = np.ma.masked_where(adpmasksmoke,vals)
                # lons_smoke = np.ma.masked_where(adpmasksmoke,lons)
                # lats_smoke = np.ma.masked_where(adpmasksmoke,lats)
                # errs_smoke = np.ma.masked_where(adpmasksmoke,errs)
                # qcpath_smoke = np.ma.masked_where(adpmasksmoke,qcpath)
                # qcall_smoke = np.ma.masked_where(adpmasksmoke,qcall)
                # obs_time_smoke = np.ma.masked_where(adpmasksmoke,obs_time)

                # vals_dust = np.ma.masked_where(adpmaskdust,vals)
                # lons_dust = np.ma.masked_where(adpmaskdust,lons)
                # lats_dust = np.ma.masked_where(adpmaskdust,lats)
                # errs_dust = np.ma.masked_where(adpmaskdust,errs)
                # qcpath_dust = np.ma.masked_where(adpmaskdust,qcpath)
                # qcall_dust = np.ma.masked_where(adpmaskdust,qcall)
                # obs_time_dust = np.ma.masked_where(adpmaskdust,obs_time)
            ...
            if self.thin > 0.0:
                ...
                #Is it an issue that there is no option to set a constant random seed here?
                #It seems fine (and possibly preferable) if the thinning
                #mask is a different set of indices for each run operationally, 
                #where each ob dataset is used once, but, for research and testing,
                #without a random seed repeated runs will assimilate
                #a slightly different set of observations. That could be an issue
                #for exact reproducibiilty. Also, for example, if a control
                #run and an experimental run were done to compare a new observation
                #operator to a currently used one,
                #then I think the results would be, at least in principle, confounded by
                #the runs not assimilating exactly the same set of obs. The effect
                #may be subtle enough that it typically wouldn't change any conclusions
                #drawn from an experiment, but it seems it would have the potential to
                #at least on rare occasions.
